...
主位依歌曲實際播放秒數輪班，不使用歌曲進度百分比。現有影片約 29 秒，因此每班依 BPM 對齊最近的 8 拍邊界，並限制在 24～28 秒內，避免影片先回到首幀才切換。拖曳歌曲進度後必須依目前秒數立即回到正確班次。
...
播放中的主位影片不要開啟 HTML `loop`，並將播放速度上限維持在 `1x`。即使音訊進度更新延遲，也應停在末幀等待接棒，不可先跳回首幀。下一位的歌曲風格影片與 `guest` 影片在接班前 10 秒才使用 `preload="auto"`，不要恢復成一次完整預載全部影片。

歌曲暫停時保留當前主位 performer、影片與畫面，並直接暫停 `<video>`；不要切回 BLACK 或待機影片。繼續播放時從同一影片位置恢復。只有歌單為空時才顯示 BLACK 待機畫面。

新主位影片載入、使用者拖曳歌曲進度或切歌時，要把影片位置同步到目前班次的 `slotElapsed`；正常播放期間不要每幀強制同步。如此可避免拖曳後沿用舊影片末幀，同時不干擾影片自然播放。
