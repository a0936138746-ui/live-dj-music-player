...
- 初始歌單保持空白，不會在重新整理或更新後重新加入測試歌曲。
- 可匯入本機 MP3 / WAV 等音訊。
- 可一次匯入多首歌曲。
- 批次匯入後會依序在背景分析全部歌曲，避免同時解碼造成瀏覽器卡頓。
- 再次匯入相同音檔時會自動略過，避免歌單出現重複歌曲。
- 匯入後會儲存在瀏覽器 IndexedDB，重新整理後仍會保留。
- BPM、能量與風格分析結果會一併保存，重新整理後不必重新掃描。
- 可搜尋歌曲、歌手、BPM。
- 可依風格篩選：科技電音、搖滾、抒情。
- 上移、下移與智慧整理後的歌單順序會保存，重新整理後維持不變。
- 可下架歌曲，之後再加回。
- 可永久刪除歌曲。
- 可一鍵清除目前歌單、下架區與本機儲存。
- 播放、進度與音量控制固定在畫面底部，桌機與手機都能直接操作。
- 自動估算 BPM、能量與歌曲風格。
...
「全部清除」會清掉目前歌單、下架區、分析快取、風格覆蓋與瀏覽器本機儲存，清除後不會重新加入展示歌曲。
